...
r = model.addVar(vtype=GRB.CONTINUOUS, name="ratio")
# endregion
...
model.addQConstr(c[index_g_min] <= c[index_l_min] - 1, "opt better greedy")

model.addQConstr(r * c[index_g_min] == c[index_l_min])
# https://support.gurobi.com/hc/en-us/community/posts/11660434512785-Fraction-with-quadratic-numerator-as-objective-function
# Since we can't have fractions and we want to optimize the function c[index_l_min]/c[index_g_min]
# we instead use an auxiliary variable to define this ratio via c[index_l_min]/c[index_g_min]
