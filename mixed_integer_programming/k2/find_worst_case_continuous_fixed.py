...
model.addQConstr(c[index_g_min] == 10e4)
model.addQConstr(c[index_l_min] <= 15e4)
...
print(jobs)
local_cost = global_cost = 0
for x, c in zip(JOB_COMBINATIONS, c_):
    if x == starting_jobs:
        local_cost = c
    if x == JOB_COMBINATIONS[index_g_min]:
        global_cost = c
    print(get_jobs(jobs, x), c)
print(local_cost / global_cost)
